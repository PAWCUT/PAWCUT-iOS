...
extension Photo {
    static var mockPhotos: [Photo] {
        ...
        let today = Date()
        for i in 0..<6 {
            let photo = Photo(fileName: "sample_image\(i+1)")
        ...
        for i in 0..<4 {
            let photo = Photo(fileName: "sample_image\(i+3)") // 3,4,5,6
        ...
        for i in 0..<5 {
            let photo = Photo(fileName: "sample_image\(i+1)") // 1,2,3,4,5
        ...
        for i in 0..<7 {
            let photo = Photo(fileName: "sample_image\(i+1)") // 1,2,3,4,5,6,7
